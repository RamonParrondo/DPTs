...
def test_create_dpt(client):
    ...
    # Verificar que el DPT se almacenó en la base de datos
    with app.app_context():
        dpt = DPT.query.first()
        assert dpt is not None
        assert dpt.title == 'Desarrollador Python'
        assert dpt.department == 'Tecnología'
        
        # Verificar que se creó la versión inicial
        version = DPTVersion.query.filter_by(dpt_id=dpt.id).first()
